    ...
    public class TelegramBot
    {
        ...
        public static InlineKeyboardMarkup CommonPairsButtons() {
            return new InlineKeyboardMarkup(
                new InlineKeyboardButton[][] {
                    new InlineKeyboardButton[] {
                        InlineKeyboardButton.WithCallbackData("BTC/AUD", "market-btcaud"),
                        InlineKeyboardButton.WithCallbackData("ETH/AUD", "market-ethaud")
                    },
                    new InlineKeyboardButton[] {
                        InlineKeyboardButton.WithCallbackData("XRP/AUD", "market-xrpaud"),
                        InlineKeyboardButton.WithCallbackData("USDT/AUD", "market-usdtaud")},
                    }
                );
        }
                ...
                try {
                    switch (cbq.Data) {
                        ...
                        case "main-cancel":
                            ...
                            SendMessage("`Cancel Order` :: ❓ Specify which pair (eg BTC-AUD):", buttons: CommonPairsButtons());
                            TGstate.commandSubStage = 1;
                            break;
                        case "market-btcaud":
                            MarketOrder_SubStage1("BTC-AUD");
                            break;
                        case "market-ethaud":
                            MarketOrder_SubStage1("ETH-AUD");
                            break;
                        case "market-xrpaud":
                            MarketOrder_SubStage1("XRP-AUD");
                            break;
                        case "market-usdtaud":
                            MarketOrder_SubStage1("USDT-AUD");
                            break;
                        case "buy-order":
                            MarketBuy_Sub();
                            break;
                    ...
                    switch (TGstate.commandChosen) {
                        ...
                        case CommandChosen.CancelOrder:
                            ...
                            if (TGstate.commandSubStage == 1) {
                                ...
                                if ((message.Text.ToUpper() == "R") && (!string.IsNullOrEmpty(TGstate.ChosenPair.Item1)) && (!string.IsNullOrEmpty(TGstate.ChosenPair.Item1))) {
                                    pairTup = TGstate.ChosenPair;
                                }
                                else {
                                    pairTup = verifyChosenPair(message.Text, "Cancel order");
                                }
        ...
        private void MarketOrder_SubStage1(string message) {
            Tuple<string, string> pairTup = verifyChosenPair(message, "Market Order");
            if (pairTup.Item1 != "") {
                TGstate.ChosenPair = pairTup;
                SendMessage("`Market Order` :: ❓ " + pairTup.Item1 + "-" + pairTup.Item2 + " chosen.  Is this a buy or a sell? (b/s)", buttons: BuySellButtons());
                TGstate.commandSubStage = 2;
            }  // no need for an else clause here, the verifychosenPair() sub handles it
        }

        private void MarketBuy_Sub() {
            ...
            try {
                ...
                foreach (KeyValuePair<string, Account> acc in accounts) {
                    if (TGstate.ChosenPair.Item2 == acc.Key) {
                        msg += "  " + acc.Key.ToUpper() + " account balance: $" + acc.Value.AvailableBalance + Environment.NewLine;
                    }
                }
            }
            catch (Exception ex) {
        ...
        private void MarketSell_Sub() {
            ...
            SendMessage(msg + Environment.NewLine + "  ❓ How much " + TGstate.ChosenPair.Item1 + " do you want to sell?");

            TGstate.commandSubStage = 40;
        }

        private async void AccountBalances_Sub() {
            Dictionary<string, Account> accounts;
            try {
                accounts = pIR.GetAccounts();
            }
        ...
        public async Task SendMessage(string message, Telegram.Bot.Types.Enums.ParseMode pMode = Telegram.Bot.Types.Enums.ParseMode.MarkdownV2, IReplyMarkup buttons = null) {
            ...
            await botClient.SendTextMessageAsync(Properties.Settings.Default.TelegramChatID, message, pMode, false, false, 0, buttons);
        }
        ...
        class TelegramState {
            ...
            public void ResetMenu() {
                ...
                TGBot.SendMessage("*IR Ticker TelegramBot Top Menu*" + Environment.NewLine +
                    "Please enter your command, or 'help' for a list of commands.", buttons: MainMenuButtons());
